...
$sql = 'SELECT d.*, c.nickname, c.foto 
        FROM depoimentos d 
        JOIN cadastro_usuarios c ON d.id_usuario = c.id'; 
$stmt = $pdo->prepare($sql);
$stmt->execute();
$depoimentos = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
<head>
    ...
    <title>Depoimentos - iNutre</title>
...
<body>
    ...
    <h2>DEPOIMENTOS</h2>
    <h3>Página de visualização dos depoimentos</h3>
            ...
            <?php foreach($depoimentos as $d): ?>
                        <div class="depoimento">
                            <div class="usuario-info">
                                <?php if(!empty($d['foto'])): ?>
                                <!--pasta + subpasta + nome da foto-->
                                    <img src="uploads/fotos/<?= htmlspecialchars($d['foto']) ?>" alt="Foto de perfil" class="foto-perfil">
                                <?php else: ?>
                                <img src="imagens/perfil-padrao.jpg" 
                                    alt="Foto de perfil padrão" 
                                    class="foto-perfil">
                                <?php endif; ?>
                            </div>
                        
                            <p><?= htmlspecialchars($d['nickname']) ?></p>
                            <p><?= htmlspecialchars($d['data_post']) ?></p>

                            <?php if (!empty($d['foto'])): ?>
                                <div class="img-depoimento">
                                    <img src="uploads/fotos/<?= htmlspecialchars($d['foto']) ?>" width="200">
                                </div>
                            <?php endif; ?>

                            <div><?= htmlspecialchars($d['conteudo']) ?></div>

                            <?php if (!empty($d['video'])): ?>
                                <video width="300" controls>
                                    <source src="uploads/videos/<?= htmlspecialchars($d['video']) ?>" type="video/mp4">
